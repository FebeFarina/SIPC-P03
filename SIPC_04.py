...
with mp_hands.Hands(
  ...
  while cap.isOpened():
    ...
    #     # cv2.destroyWindow('kon')
      ...
      #Modo pintar
      if fingersUp == [1,1,0,0,1] and not paintMode:
        paintMode = True
      elif fingersUp == [0,1,0,0,1] and paintMode:
        paintMode = False
      if fingersUp == [0,1,0,0,0] and paintMode:
        cv2.circle(image, (int(lm[tipIds[1]].x*w),int(lm[tipIds[1]].y*h)), 10, colour, -1)
        cv2.circle(imgCanvas, (int(lm[tipIds[1]].x*w),int(lm[tipIds[1]].y*h)), 10, colour, -1)
      if paintMode:
        cv2.putText(image, "Paint mode", (20,450),cv2.FONT_HERSHEY_SIMPLEX,1,colour, thickness = 2)
      else:
        cv2.putText(image, "Paint mode", (20,450),cv2.FONT_HERSHEY_SIMPLEX,1,(128,128,128), thickness = 2)
      cv2.putText(image, str(sum(fingersUp)), (20,400),cv2.FONT_HERSHEY_SIMPLEX,2,(255,0,0), thickness = 5) 
      if 625 > int(lm[tipIds[2]].x*w) > 565 and int(lm[tipIds[2]].y*h) < 70:
        imgCanvas.fill(0)
      elif 500 > int(lm[tipIds[2]].x*w) > 425 and int(lm[tipIds[2]].y*h) < 70:
        colour = (255,0,0)
      elif 365 > int(lm[tipIds[2]].x*w) > 290 and int(lm[tipIds[2]].y*h) < 70:
        colour = (0,255,0)
      elif 220 > int(lm[tipIds[2]].x*w) > 145 and int(lm[tipIds[2]].y*h) < 70:
        colour = (0,0,255)
      elif 80 > int(lm[tipIds[2]].x*w) > 20 and int(lm[tipIds[2]].y*h) < 70:
        colour = (255,255,255)
    ...
    header = cv2.imread('Menu.png')
    image[0:60, 0:640] = header
